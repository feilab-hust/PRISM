...
class Conv(nn.Module):
    def __init__(self, dim_i, dim_o, k, groups=1, bias=True, device=None):
        ...
        p = (k - 1) // 2
        self.conv_1 = nn.Conv3d(dim_i, dim_o, kernel_size=(1, k, k), stride=1, padding=(0, p, p), groups=groups, bias=bias, device=device)
        self.conv_2 = nn.Conv3d(dim_o, dim_o, kernel_size=(k, 1, 1), stride=1, padding=(p, 0, 0), groups=groups, bias=bias, device=device)
...
#         super(Conv, self).__init__()
#         p = (k - 1) // 2
#         self.conv = nn.Conv3d(dim_i, dim_o, kernel_size=(k, k, k), stride=1, padding=(p, p, p), groups=groups, bias=bias, device=device)
#
#     def forward(self, x):
#         return self.conv(x)
...
class FeedForward(nn.Module):
    def __init__(self, dim, ffn_expansion_factor, bias):
        ...
        self.conv_init = nn.Conv3d(dim, hidden_features, 1, bias=bias)

        # self.conv1_1 = nn.Conv3d(self.dim_sp, self.dim_sp, kernel_size=3, padding=1, groups=self.dim_sp, bias=bias)
        # self.conv1_2 = nn.Conv3d(self.dim_sp, self.dim_sp, kernel_size=5, padding=2, groups=self.dim_sp, bias=bias)
        # self.conv1_3 = nn.Conv3d(self.dim_sp, self.dim_sp, kernel_size=7, padding=3, groups=self.dim_sp, bias=bias)
        ...
        self.gelu = nn.GELU()
        self.conv_fina = nn.Conv3d(hidden_features, dim, 1, bias=bias)
...
class ResidualGroup(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type, num_blocks, LN_flag=True, drop_path_rates=None):
        ...
        self.pw = nn.Conv3d(dim, dim, kernel_size=1, bias=bias)
        # self.dw = nn.Conv3d(dim, dim, kernel_size=3, padding=1, groups=dim, bias=bias)
...
class DeepEncoder_Trans(nn.Module):
    ...
    def __init__(self,
        ...
        self.final_conv = nn.Sequential(
            nn.Conv3d(num_features, num_features, kernel_size=1, bias=bias),
            # Conv(num_features, num_features, k=3, groups=num_features, bias=bias),
            nn.Conv3d(num_features, num_features, kernel_size=3, stride=1, padding=1, groups=num_features, bias=bias)
        )
...
class AxialDownsample(nn.Module):
    def __init__(self, dim):
        super().__init__()
        # self.down = nn.Conv3d(dim, dim, kernel_size=(2, 1, 1), stride=(2, 1, 1), padding=0)
        self.down = nn.AvgPool3d(kernel_size=(2, 1, 1), stride=(2, 1, 1))

    def forward(self, x):
        return self.down(x)


class AxialUpsample(nn.Module):
    def __init__(self, dim):
        super().__init__()
        # self.up = nn.ConvTranspose3d(dim, dim, kernel_size=(2, 1, 1), stride=(2, 1, 1), padding=0)

        self.upsample = lambda x: F.interpolate(x,
                                                scale_factor=(2, 1, 1),
                                                mode='trilinear')
        self.conv = Conv(dim_i=dim, dim_o=dim, k=3, groups=dim)
...
class DeepEncoder_AxialTransUNet(nn.Module):
    def __init__(self,
        ...
        for i in range(self.num_layers - 1):
            ...
            self.down.append(AxialDownsample(dim))
            idx += num_blocks[i]
        ...
        for i in range(self.num_layers - 1):
            self.up.append(AxialUpsample(dim))
            self.decoder.append(
        ...
        self.skip_conv = nn.Sequential(nn.Conv3d(dim, dim, kernel_size=1, bias=bias),
