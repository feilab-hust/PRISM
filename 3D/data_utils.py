...
def get_3d_dataloaders_single(x_blur_dir, batch_size=8, crop_size=64, crop_depth=32, shuffle=False,
                            top_percent=0.1, num_per_img=200):
    """
    高效地读取三维体数据，进行随机裁剪、高方差筛选并生成批次。

    该函数直接在内存中执行所有操作，避免了 Dataset/DataLoader 的 I/O 瓶颈。

    输出形状: [num_batches, batch_size, 1, depth, height, width]
    """
    # ------------------ 1. 文件 I/O 和初始化 ------------------
    ...
    print(f"找到 {len(image_names)} 个三维体文件，开始预处理...")
    ...
    for img_idx, img_name in enumerate(image_names):
        ...
        # 优化 I/O: 一次性读取整个堆栈
        # 形状: [D_orig, H_orig, W_orig]
        z_stack_np = imread(x_img_path)[:, :, :].astype(np.float32)

        D_orig, H_orig, W_orig = z_stack_np.shape

        if D_orig < crop_depth or H_orig < crop_size or W_orig < crop_size:
            print(f"警告: 文件 {img_name} 尺寸 ({D_orig}x{H_orig}x{W_orig}) 小于裁剪尺寸，跳过。")
            continue

        # 转换为 PyTorch 张量，并添加通道维度: [1, D, H, W]
        ...
        for _ in range(num_per_img):
            ...
            # 裁剪后的形状: [1, crop_depth, crop_size, crop_size]
        ...
        # 将所有裁剪块堆叠成一个张量 [num_per_img, 1, D, H, W]
        ...
        values = patches_tensor[:, 0, d_q:3 * d_q, h_q:3 * h_q, h_q:3 * h_q].std(dim=(-3, -2, -1))
    ...
    # 堆叠所有选定的裁剪块 [Total_Patches, 1, D, H, W]
    ...
    # 划分成批次 [num_batches, batch_size, 1, depth, height, width]
    final_batches = torch.stack([
