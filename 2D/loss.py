...
from utils import SSIM, TVLoss, HessianLoss, L1SparsityLoss, DarkChannelPrior2D, normalize_to_01, CombinedLoss, _create_2d_split_sigmoid_lowpass_mask
...
class CustomLoss_single(nn.Module):
    def __init__(self, spatial_weight=1, ssim_weight=0.04, fourier_weight=0.05, loss_type='both', fourier_flag=False,
                 ...
                 fourier_magnitude_transition_width_lateral=0.25, H=128, W=128, device=torch.device('cpu')):
        ...
        self.SSIM = SSIM()
        self.mask = _create_2d_split_sigmoid_lowpass_mask(H, W, fourier_magnitude_cutoff_ratio_lateral, fourier_magnitude_transition_width_lateral, device).unsqueeze(0).unsqueeze(0)
    ...
    def forward(self, predicted_blur_x, blurred_x):
        ...
        if self.fourier_flag:
            fourier_loss_x = self.compute_2d_fourier_loss(predicted_blur_x, blurred_x)
        else:
    ...
    def compute_2d_fourier_loss(self, pred, target):
        pred_fft = torch.fft.fftshift(torch.fft.fftn(pred, dim=(-2, -1)), dim=(-2, -1))
        target_fft = torch.fft.fftshift(torch.fft.fftn(target, dim=(-2, -1)), dim=(-2, -1))
...
class FinalLoss_single(nn.Module):
    def __init__(self, spatial_weight=1, high_weight=1, ssim_weight=0.04, fourier_weight=0.05, fourier_weight_high=0.05, hessian_weight=10, tv_weight=10,
                 sparsity_weight=0.1, dark_weight=0.5, loss_type='L1', window_size=7, alpha=0.5, fourier_flag=False,
                 fourier_magnitude_cutoff_ratio_lateral=0.4,  # 横向截止比例
                 fourier_magnitude_transition_width_lateral=0.25, H=128, W=128, device=torch.device('cpu'),
        ...
        self.custom_loss = CustomLoss_single(spatial_weight, ssim_weight, fourier_weight, loss_type, fourier_flag,
                                             fourier_magnitude_cutoff_ratio_lateral, fourier_magnitude_transition_width_lateral, H, W, device)
        self.custom_loss_high = CustomLoss_single(spatial_weight, ssim_weight, fourier_weight_high, loss_type, fourier_flag,
                                             fourier_magnitude_cutoff_ratio_lateral2, fourier_magnitude_transition_width_lateral2, H, W, device)
        self.tv_loss = TVLoss()
        self.hessian_loss = HessianLoss()
        self.l1_sparsity_loss = L1SparsityLoss()
        self.dark_loss = DarkChannelPrior2D(window_size=window_size, alpha=alpha)
    ...
    def forward(self, predicted_raw, predicted_raw_high, blurred, blurred_high, y_pred, predicted_blur):
        ...
        # 计算 TV 和 Hessian 损失
        tv_loss_xy = self.tv_loss(y_pred)
        ...
        total_loss += (self.hessian_weight * hessian_loss +
                       self.tv_weight * tv_loss_xy +
                       self.sparsity_weight * sparsity_loss +
                       self.dark_weight * dark_loss)

        total_loss_normal += (self.hessian_weight * hessian_loss.item() +
                              self.tv_weight * tv_loss_xy.item() +
                              self.sparsity_weight * sparsity_loss.item() +
                              self.dark_weight * dark_loss.item())

        return total_loss_normal, total_loss, spatial_loss_low, ssim_loss_low, fourier_loss_low, spatial_loss_high, ssim_loss_high, fourier_loss_high, tv_loss_xy, hessian_loss, sparsity_loss, dark_loss


class FinalLoss_single_blur(nn.Module):
    def __init__(self, spatial_weight=1, ssim_weight=0.04, fourier_weight=0.05, hessian_weight=10, tv_weight=10,
                 sparsity_weight=0.1, dark_weight=0.5, loss_type='L1', window_size=7, alpha=0.5, fourier_flag=False,
                 fourier_magnitude_cutoff_ratio_lateral=0.4,  # 横向截止比例
                 fourier_magnitude_transition_width_lateral=0.25, H=128, W=128, device=torch.device('cpu')):
    ...
    def forward(self, predicted_blur, blurred, y_pred):
        ...
        return total_loss_normal, total_loss, spatial_loss, ssim_loss, fourier_loss, tv_loss_xy, hessian_loss, sparsity_loss, dark_loss
